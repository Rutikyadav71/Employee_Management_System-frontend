      ...
      <div style={{background:"var(--bg-elevated)",border:"1px solid var(--border-bright)",
        ...
        {[
          ...
          <div key={ctrl.label} style={{marginBottom:14}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <button onClick={ctrl.decBtn} style={{background:"var(--bg-hover)",border:"1px solid var(--border)",
                borderRadius:6,padding:"5px 7px",cursor:"pointer",color:"var(--text-2)",display:"flex"}}>{ctrl.decIcon}</button>
              <input type="range" {...ctrl.slider} style={{flex:1,accentColor:"var(--accent)",cursor:"pointer"}}/>
              <button onClick={ctrl.incBtn} style={{background:"var(--bg-hover)",border:"1px solid var(--border)",
                borderRadius:6,padding:"5px 7px",cursor:"pointer",color:"var(--text-2)",display:"flex"}}>{ctrl.incIcon}</button>
            </div>
          </div>
        ))}
        ...
        <div style={{display:"flex",gap:10,marginTop:20}}>
          <button onClick={()=>{setScale(1);setRotation(0);setOffset({x:0,y:0});}}
            style={{padding:"10px 16px",background:"var(--bg-hover)",border:"1px solid var(--border-bright)",
              color:"var(--text-2)",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:500}}>Reset</button>
          <button onClick={handleSave} disabled={saving}
            style={{flex:1,background:"var(--accent)",color:"white",border:"none",padding:"10px 16px",
            ...
            {saving?<><div style={{width:14,height:14,border:"2px solid rgba(255,255,255,0.3)",
              borderTopColor:"white",borderRadius:"50%",animation:"spin .7s linear infinite"}}/> Uploading...</>
...
export default function ProfileImageUpload({ empId, currentUrl, name, onUpload, size=80, editable=true, customUploadFn }) {
    ...
    try {
      ...
      if (customUploadFn) {
        ...
        const res = await axios.post(
          `http://localhost:8080/api/upload/profile/${empId}`,
          fd,
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
      ...
      {error&&<div style={{fontSize:12,color:"var(--red)",padding:"6px 12px",background:"var(--red-glow)",
        borderRadius:6,border:"1px solid rgba(239,68,68,0.3)",maxWidth:260,textAlign:"center",lineHeight:1.5}}>{error}</div>}
